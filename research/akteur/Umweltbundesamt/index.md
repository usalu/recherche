---
entity: "akteur"
id: "Umweltbundesamt"
title: "Umweltbundesamt"
build_status: "promoted_phase42"
legacy_paths:
  - "akteur\\01_oeffentliche_institutionen_foerderung\\Umweltbundesamt.md"
node_kind: "core"
---

# Umweltbundesamt

## Legacy Content

### Legacy Source: akteur\01_oeffentliche_institutionen_foerderung\Umweltbundesamt.md

- Map action: move_as_core
- Target role in map: primary
- Raw mapped target: akteur/Umweltbundesamt
- Original primary target: akteur/Umweltbundesamt
- Original secondary targets: akteurtyp/oeffentliche_institutionen_foerderung

## Verknüpfungen

**Übergeordnete Themen**
- Ressourcenschutz und Abfallvermeidung
- Circular Economy im Bauwesen
- Umweltpolitische Instrumente

**Verwandte Dateien**
- `akteur/BBSR_Zukunft_Bau.md`
- `akteur/VDI_ZRE.md`
- `recht/Kreislaufwirtschaftsgesetz.md`
- `methode/Abfallhierarchie.md`
- `material/Bau_und_Abbruchabfaelle.md`

**Akteurstyp**
- Umweltpolitischer und wissenschaftlicher Bundesakteur

## Kurzüberblick zur Kategorie

**Umweltbundesamt** ist im Kontext „Entwerfen mit Bestand“ / Wiederverwendung in der Architektur ein Umweltpolitischer und wissenschaftlicher Bundesakteur. Das Umweltbundesamt ist die zentrale Umweltbehörde des Bundes. Im ReUse-Kontext liefert es Daten, Studien und Empfehlungen zu Ressourcenschonung, Bauabfällen, Klimaschutz und zirkulärer Bauwirtschaft. Es ist kein Baustellenakteur, aber eine sehr belastbare Quelle für politische und ökologische Einordnung.

Für das Repo ist dieser Akteur besonders wichtig, weil die Wiederverwendung von Bauteilen nicht nur durch einzelne Produkte entsteht, sondern durch ein Zusammenspiel aus politischem Rahmen, Planungskultur, Daten, Materiallogistik, Rückbau, Qualitätssicherung und Nachfrage. Die Datei ist als Akteursprofil zu lesen: Sie beschreibt Rolle, Nutzen, Grenzen und sinnvolle Querverbindungen, ersetzt aber keine projektspezifische Prüfung von Recht, Technik, Schadstoffen, Kosten oder Verfügbarkeit.

## Zentrale Unterthemen

- Transformation zu einer zirkulären Bauwirtschaft
- Bau- und Abbruchabfälle
- Abfallhierarchie
- Ressourceneffizienz und politische Instrumentenanalyse
- Abgrenzung von ReUse, Recycling und Verwertung

## Wichtige Dateien dieser Kategorie mit je 1–3 Sätzen Einordnung

- methode/Abfallhierarchie.md — sollte UBA-Quellen als Grundlage nutzen.
- recht/Kreislaufwirtschaftsgesetz.md — verbindet ReUse mit Abfallvermeidungslogik.
- material/Bau_und_Abbruchabfaelle.md — kann UBA-Daten zur Bauabfallproblematik aufnehmen.

Weitere Anschlussdateien sollten den Akteur nicht nur erwähnen, sondern prüfen, welche konkrete Funktion er im ReUse-Prozess übernimmt: politischer Druck, technische Bewertung, Marktplatz, physisches Lager, Planungskompetenz, Forschung, Zertifizierung oder kommunale Infrastruktur.

## Relevanz für Wiederverwendung / Entwerfen mit Bestand

- Stützt die Priorität von Vermeidung und Wiederverwendung gegenüber Recycling und Entsorgung.
- Hilft, hohe Recyclingquoten im Bauwesen kritisch einzuordnen, wenn Materialien dennoch downgecycelt oder Primärmaterial nicht ersetzt werden.
- Bietet Quellen für Abfallrecht, Ressourcenschutz, politische Instrumente und öffentliche Beschaffung.

Die Einordnung ist bewusst nicht auf „Bauteile kaufen“ reduziert. Gerade in Deutschland entstehen ReUse-Potenziale häufig vor dem eigentlichen Materialhandel: durch Erhaltungsentscheidungen, frühe Zielvereinbarungen, selektiven Rückbau, Gebäuderessourcenpässe, kommunale Beschaffung, lokale Materialorte und eine Planungskultur, die mit Verfügbarkeit, Patina und Unregelmäßigkeit umgehen kann.

## Querverbindungen zu anderen Kategorien

- Zu BBSR: UBA ist umweltpolitisch, BBSR bauforschungsnah.
- Zu VDI ZRE: UBA liefert Rahmen und Daten, VDI ZRE stärker Praxiswerkzeuge für Unternehmen.
- Zu Architects for Future: politische Forderungen können mit UBA-Daten unterlegt werden.

Für die Arbeit im Repo sollte der Akteur mit Methoden-, Werkzeug-, Material-, Rechts- und Wirtschaftlichkeitsdateien verknüpft werden. Besonders wichtig ist die Unterscheidung zwischen:
- **Diskurs / Policy**: schafft Legitimation und politische Forderungen.
- **Daten / Bewertung**: macht Ressourcen sichtbar und vergleichbar.
- **Markt / Logistik**: bringt Angebot und Nachfrage zusammen.
- **Planung / Entwurf**: integriert verfügbare Bauteile in Räume, Tragwerke und Details.
- **Nachweis / Recht**: entscheidet, ob ein Bauteil genehmigungs- und haftungsfähig wiederverwendet werden kann.

## Offene Lücken / Ausbaufelder

- UBA-Empfehlungen sind nicht automatisch verbindliches Recht.
- Die Datenlage zur tatsächlichen direkten Bauteilwiederverwendung bleibt lückenhaft.
- Umweltpolitische Ziele müssen mit Baurecht, Vergabe, Haftung und Marktstrukturen verbunden werden.

Zusätzlich sollte bei jeder späteren Vertiefung geprüft werden, ob die Angaben auf offiziellen Quellen, unabhängigen Evaluationen, Projektpublikationen oder Eigenkommunikation beruhen. Eigenangaben von Plattformen, Büros und Herstellern sind nützlich, müssen aber bei CO₂-Einsparungen, Wiederverwendungsquoten, Preisen und Skalierbarkeit kritisch markiert werden.

## Quellen / Bezugslogik

- https://www.umweltbundesamt.de/
- https://www.umweltbundesamt.de/publikationen/transformation-zu-einer-zirkulaeren-bauwirtschaft
- https://www.umweltbundesamt.de/themen/abfall-ressourcen/abfallwirtschaft/abfallarten/bauabfaelle
- https://www.umweltbundesamt.de/themen/wirtschaft-konsum/produkte/bauprodukte
