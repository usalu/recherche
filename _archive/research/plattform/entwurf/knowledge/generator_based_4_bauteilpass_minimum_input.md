# Generator-Based Minimum Input Strategy for the Bauteilpass Interface

**Purpose**  
This document defines how a Bauteilpass interface can display rich reclaimed-component information from a very small amount of input by using **typology-specific Generators**.

**Project framing**  
The logic is adapted to the Abbau/Aufbau reuse workflow, but generalized for other reclaimed-component systems.

**Core correction**  
The system does not simply “extract” structural, energy, connector, port, semantic, and logistics representations.  
Instead, these are produced by **Generators**.

A Generator is a preprogrammed typology-based logic module.  
It receives minimum input plus available base geometry and produces the required representations and geometry-tied data.
...
The user or project context provides only a small amount of data:
...
```text
The user provides identity, typology, material, source context, and base geometry.
...
A **Generator** is a project-defined logic module.

It is not an AI guess.  
It is a deterministic or semi-deterministic routine based on:
...
Slab Generator
│
├── detects top / bottom / edges
├── classifies span direction
├── creates bearing-edge zones
├── creates slab-edge-bearing ports
├── creates service-zone candidates
├── creates envelope-face candidates
├── calculates mass and self-weight
├── creates LCA precheck data
└── marks missing structural proof as engineering_required
```

For a mushroom column typology:

```text
Mushroom Column Generator
│
├── detects shaft
├── detects capital / mushroom head
├── creates column-base port
├── creates column-head-bearing port
├── detects punching-sensitive slab interface
├── creates lifting / standing-storage logic
└── marks local bearing and punching checks as engineering_required
```
...
```text
Generated data:
Can be produced from typology + geometry + defaults.

Evidence data:
Must come from tests, scans, reports, or inspection.

Unknown data:
Must remain unknown or engineering_required.
```
...
Generators are built on component typologies.
...
typology:
  ...
  expected_geometry:
    aspect_ratio: thin_horizontal_plate
...
height = vertical principal dimension for column / wall
```
...
## Avoided new material potential
...
## Rough concrete U-value precheck
...
This is only a precheck. Final energy proof requires the full assembly.

---

# 5. Piece Detail Panel Structure

The interface still follows the same structure:
...
Each section below is written as:
...
thumbnail =
existing preview representation
or generated viewport capture from base_geometry
