---
id: "Werner_Sobek"
entity: "akteur"
node_kind: "core"
migration_status: "migrated_phase3_core_entities"
title: "Werner Sobek"
source_count: 1
legacy_paths:
  - "akteur\\04_planung_architektur_ingenieurwesen\\Werner_Sobek.md"
raw_targets:
  - "akteur/Werner_Sobek"
migration_actions:
  - "move_as_core"
risk_flags:
---
# Werner Sobek

## Migration

- Canonical target: akteur/Werner_Sobek
- Legacy source count: 1
- Semantic note: Akteurprofil. Kann spaeter mit Rollen, Beteiligungen, Orten und Plattformbetreiberschaft verknuepft werden.

## Legacy Content

### Legacy Source: akteur\04_planung_architektur_ingenieurwesen\Werner_Sobek.md

- Map action: move_as_core
- Target role in map: primary
- Raw mapped target: akteur/Werner_Sobek
- Original primary target: akteur/Werner_Sobek
- Original secondary targets: akteurtyp/planung_architektur_ingenieurwesen

## Verknüpfungen

**Übergeordnete Themen**
- Urban Mining Index
- Gebäuderessourcenpass
- Leichtbau, Tragwerk und rückbaubare Konstruktion

**Verwandte Dateien**
- `werkzeug/Urban_Mining_Index.md`
- `werkzeug/DGNB_Gebaeuderessourcenpass.md`
- `methode/Design_for_Disassembly.md`
- `material/Tragwerk_Wiederverwendung.md`
- `wirtschaft/Materialwert_Gebaeude.md`

**Akteurstyp**
- Ingenieur- und Planungsakteur für ressourceneffizientes Bauen

## Kurzüberblick zur Kategorie

**Werner Sobek** ist im Kontext „Entwerfen mit Bestand“ / Wiederverwendung in der Architektur ein Ingenieur- und Planungsakteur für ressourceneffizientes Bauen. Werner Sobek ist ein deutsches Ingenieur- und Architekturbüro mit starker Rolle in Tragwerksplanung, Fassadenplanung, nachhaltigem Bauen und Forschung. Für ReUse ist der Akteur relevant durch Konzepte wie Urban Mining Index, Gebäuderessourcenpass und rückbaubare, materialeffiziente Konstruktionen.

Für das Repo ist dieser Akteur besonders wichtig, weil die Wiederverwendung von Bauteilen nicht nur durch einzelne Produkte entsteht, sondern durch ein Zusammenspiel aus politischem Rahmen, Planungskultur, Daten, Materiallogistik, Rückbau, Qualitätssicherung und Nachfrage. Die Datei ist als Akteursprofil zu lesen: Sie beschreibt Rolle, Nutzen, Grenzen und sinnvolle Querverbindungen, ersetzt aber keine projektspezifische Prüfung von Recht, Technik, Schadstoffen, Kosten oder Verfügbarkeit.

## Zentrale Unterthemen

- Gebäuderessourcenpass
- Urban Mining Index
- Design for Disassembly
- Leichtbau und Materialeffizienz
- Tragwerk, Fassade und Gebäudehülle als Materiallager

## Wichtige Dateien dieser Kategorie mit je 1–3 Sätzen Einordnung

- werkzeug/Urban_Mining_Index.md — Bewertungslogik für Kreislaufpotenziale und Ressourcennutzung.
- werkzeug/DGNB_Gebaeuderessourcenpass.md — Dokumentation von Materialeinsatz und Rückbaupotenzial.
- material/Tragwerk_Wiederverwendung.md — untersucht die besonders anspruchsvolle Wiederverwendung tragender Bauteile.

Weitere Anschlussdateien sollten den Akteur nicht nur erwähnen, sondern prüfen, welche konkrete Funktion er im ReUse-Prozess übernimmt: politischer Druck, technische Bewertung, Marktplatz, physisches Lager, Planungskompetenz, Forschung, Zertifizierung oder kommunale Infrastruktur.

## Relevanz für Wiederverwendung / Entwerfen mit Bestand

- Tragwerk und Gebäudehülle enthalten große Material- und Emissionsmengen; ihr ReUse-Potenzial ist strategisch wichtig.
- Methoden wie Urban Mining Index und Ressourcenpass machen Materialmengen, Rückbaubarkeit und Kreislaufpotenziale bewertbar.
- Ingenieurplanung entscheidet früh über Verbindungsarten, Materialeffizienz und spätere Demontierbarkeit.

Die Einordnung ist bewusst nicht auf „Bauteile kaufen“ reduziert. Gerade in Deutschland entstehen ReUse-Potenziale häufig vor dem eigentlichen Materialhandel: durch Erhaltungsentscheidungen, frühe Zielvereinbarungen, selektiven Rückbau, Gebäuderessourcenpässe, kommunale Beschaffung, lokale Materialorte und eine Planungskultur, die mit Verfügbarkeit, Patina und Unregelmäßigkeit umgehen kann.

## Querverbindungen zu anderen Kategorien

- Zu DGNB: Ressourcenpässe und Zertifizierung können methodisch verbunden werden.
- Zu Madaster: beide arbeiten an Materialtransparenz, aber mit unterschiedlicher Software- und Beratungslogik.
- Zu Tragwerksplanung: ReUse benötigt Nachweise, Resttragfähigkeit und konservative Risikobewertung.

Für die Arbeit im Repo sollte der Akteur mit Methoden-, Werkzeug-, Material-, Rechts- und Wirtschaftlichkeitsdateien verknüpft werden. Besonders wichtig ist die Unterscheidung zwischen:
- **Diskurs / Policy**: schafft Legitimation und politische Forderungen.
- **Daten / Bewertung**: macht Ressourcen sichtbar und vergleichbar.
- **Markt / Logistik**: bringt Angebot und Nachfrage zusammen.
- **Planung / Entwurf**: integriert verfügbare Bauteile in Räume, Tragwerke und Details.
- **Nachweis / Recht**: entscheidet, ob ein Bauteil genehmigungs- und haftungsfähig wiederverwendet werden kann.

## Offene Lücken / Ausbaufelder

- Bewertungsmethoden garantieren keine tatsächliche spätere Wiederverwendung.
- Tragende Bauteile haben hohe Nachweisanforderungen und sind schwieriger als Ausbauprodukte.
- Gebäuderessourcenpässe brauchen langfristige Datenpflege und Aktualisierung.

Zusätzlich sollte bei jeder späteren Vertiefung geprüft werden, ob die Angaben auf offiziellen Quellen, unabhängigen Evaluationen, Projektpublikationen oder Eigenkommunikation beruhen. Eigenangaben von Plattformen, Büros und Herstellern sind nützlich, müssen aber bei CO₂-Einsparungen, Wiederverwendungsquoten, Preisen und Skalierbarkeit kritisch markiert werden.

## Quellen / Bezugslogik

- https://www.wernersobek.com/
- https://www.wernersobek.com/de/themen/gebaeuderessourcenpass/
- https://urban-mining-index.de/
- https://www.dgnb.de/de/nachhaltiges-bauen/zirkulaeres-bauen/gebaeuderessourcenpass
