---
id: "Maison_Vignette_Auderghem"
entity: "fallstudie"
node_kind: "core"
migration_status: "migrated_phase4_case_graph"
title: "Maison Vignette, Auderghem – Fallstudie Direct Reuse"
bauobjekt:
  - "Maison_Vignette_Auderghem"
legacy_paths:
  - "Gebäude\\Maison_Vignette_Auderghem.md"
projekt:
  - "Maison_Vignette_Auderghem"
reuse_chain_detected: "False"
---
# Maison Vignette, Auderghem – Fallstudie Direct Reuse

## Migration

- Fallstudie ID: Maison_Vignette_Auderghem
- Legacy source count: 1
- Generated project: Maison_Vignette_Auderghem
- Generated bauobjekt: Maison_Vignette_Auderghem
- Extracted reuse_einsatz rows: 9
- Extracted datenpunkt rows: 12
- Extracted entity mapping rows: 22
- Reuse chain detected: False

## Legacy Content

### Legacy Source: Gebäude\Maison_Vignette_Auderghem.md

- Map action: split_into_case_graph
- Primary target: fallstudie/Maison_Vignette_Auderghem
- Secondary targets: projekt/Maison_Vignette_Auderghem; bauobjekt/<from_content>; reuse_einsatz/<per_component>
- Risk flags: do_not_treat_file_as_single_gebaeude_only

# Maison Vignette, Auderghem – Fallstudie Direct Reuse

**Recherchezeitpunkt:** 2026-05-06  
**Sprache:** Deutsch  
**Grundregel angewandt:** Gezählt werden nur tatsächlich wiederverwendete Bau-, Hüll-, Raum-, Technik- oder fest eingebaute Konstruktionselemente. Biosourcierte Materialien wie Holz, Stroh, Hanf-Kalk werden dokumentiert, aber **nicht** als Direct Reuse gezählt, wenn sie neu sind.

---

## 1. EINORDNUNG

- **Entscheidung:** VERGLEICHSFALL
- **Bewertung:** ★★★☆☆
- **Begründung:** Neubau eines Einfamilienhauses mit belegten wiederverwendeten Bauteilen: Ziegel-Fassadenclaustra, Wand-/Bodenfliesen, Blausteinplatten und Sanitär. Das Haupttragwerk ist jedoch neu aus Holz/Stroh und daher kein tragender Reuse-Fall.
- **Vertrauensgrad:** belegt
- **Warnung Bestandserhalt:** nein – Neubau auf unbebautem Grundstück.
- **Warnung Möbel/Dekoration:** nein – gezählt werden feste Bauteile/Fassaden, Böden, Sanitär; lose Gegenstände werden nicht gewertet.
- **Projektstatus:** gebaut

---

## 2. ENTITÄTEN-MAPPING

| Entität | Wert | Beziehung zur Fallstudie | Quelle/Beleg | Vertrauensgrad | Anmerkung |
|---|---|---|---|---|---|
| Fallstudie | Maison Vignette / Vignette House | Untersuchter Fall | [S1], [S2], [S3] | belegt | Ausgangsliste und mehrere Projektquellen. |
| Gebäude | Einfamilienhaus in Auderghem | Zielgebäude | [S2], [S3] | belegt | Neubau auf unbebautem Grundstück. |
| Projekt | Neubau mit Reuse + bio-/geobasierten Materialien | Projektlogik | [S2], [S3], [S4] | belegt | Mischung aus Strategien. |
| Ort | Rue de la Vignette, Auderghem, Brüssel, Belgien | Standort | [S2] | belegt | Adresse nur auf Straßenebene veröffentlicht. |
| People | Karbon’ architecture & urbanisme | Architekt | [S2], [S3] | belegt | Entwurf. |
| People | Pierre Stoffel / BESP Stoffel & Partners; Sofiane Boudahri | Tragwerks-/Engineering-Unterstützung | [S2], [S3] | belegt | Unterschiedliche Quellen nennen Pierre Stoffel bzw. BESP/Sofiane Boudahri. |
| Bauherr | Private owner | Bauherr | [S3] | belegt | Privater Auftrag. |
| Bauteil | Large/Grote Boomse Steen Ziegel | Reuse-Fassadenbauteil | [S2], [S3] | belegt | 3.000 Stück / 36 m². |
| Bauteil | Wandfliesen aus Solvay-Gebäude | Reuse-Innenausbau | [S2] | belegt | 21 m². |
| Bauteil | Terrakotta-Bodenfliesen | Reuse-Boden | [S2] | belegt | 13,5 m². |
| Bauteil | Blausteinplatten | Reuse-Boden / Terrasse / Eingang | [S2] | belegt | 40 m² über 2emain.be. |
| Bauteil | Sanitärausstattung | Reuse-Technik/fester Ausbau | [S2] | belegt | Waschbecken/Vasen/Atelierbecken von Rotor DC. |
| Bauteilbörse | Franck Bricks; Rotor DC; 2emain.be | Lieferquellen | [S2] | belegt | Reuse-Markt/Dealer + Kleinanzeigen. |
| Material | Holz, Stroh, Hanf-Kalk, Naturputz | neue bio-/geobasierte Materialien | [S2], [S3] | belegt | Nicht als Direct Reuse gezählt. |
| Reuse-Strategie | ex-situ Bauteilwiederverwendung | zentrale Strategie | [S2], [S3] | belegt | Mehrere Bauteilgruppen. |
| Tragwerkssystem | Holzrahmen / Holzstützen und -träger, Strohballen-Füllung | Haupttragwerk, nicht Reuse | [S2], [S3] | belegt | Neubau-Tragwerk. |
| Verbindung | Ziegel-Claustra ohne zusätzliche metallische Aufhängung | besondere Fassadenlösung | [S2] | belegt | Breiter Ziegeltyp ermöglicht konstruktive Lösung. |
| Förderprogramm | Be Exemplary | Anerkennung / Projektförderkontext | [S2], [S5] | belegt | Projekt als exemplarisch anerkannt. |
| Bericht | FCRBE 32 detailed project sheets | Sekundäranalyse | [S3] | belegt | Enthält Projektfläche, Dauer, Akteure. |
| Kennwert | 3.000 Ziegel / 36 m² Fassade | Mengen | [S2], [S3] | belegt | Zentrales Reuse-Kennwertpaar. |
| Prüfung | unbekannt | Prüf-/Zulassungsdaten | keine Quelle gefunden | unbekannt | Keine Prüfprotokolle. |
| Wirtschaft | unbekannt | Kostenwirkung | keine Quelle gefunden | unbekannt | Keine Kostenkennwerte gefunden. |

### Vorgeschlagene neue Entität

| Neue Entität | Warum nötig? | Beispiel aus dem Fall | Beziehung zu bestehenden Entitäten |
|---|---|---|---|
| Materialallianz | Der Fall kombiniert Reuse mit bio-/geobasierten neuen Materialien; die Reuse-Bewertung muss beides trennen. | Reuse-Ziegel + neue Holz-/Stroh-/Hanfkalk-Konstruktion. | Verknüpft Material, Reuse-Strategie, Tragwerkssystem und Bewertung. |
| Privatkleinanzeige / informeller Reuse-Markt | 2emain.be ist keine klassische Bauteilbörse, aber reale Beschaffungsquelle. | 40 m² Blausteinplatten über 2emain.be. | Unterform von Bauteilbörse / Beschaffung. |

---

## 3. FALLSTUDIE

- **Name:** Maison Vignette / Vignette House
- **Ort:** Rue de la Vignette, Auderghem, Brüssel, Belgien
- **Gebäude:** Einfamilienhaus, Neubau, zwei Volumen um einen Patio
- **Projekt:** Neubau auf unbebautem Grundstück mit Reuse- und bio-/geobasierten Materialien
- **Beteiligte People / Akteure:** private Bauherrschaft; Karbon’ architecture & urbanisme; Pierre Stoffel / BESP Stoffel & Partners; Sofiane Boudahri; Gauthier Nagant; 3ALJ Construct; Franck Bricks; Rotor DC
- **Architekt:** Karbon’ architecture & urbanisme
- **Tragwerksplaner:** Pierre Stoffel / BESP Stoffel & Partners; Sofiane Boudahri laut FCRBE
- **Bauherr:** private owner
- **Zeitraum:** 2018–2020 laut FCRBE; Opalis nennt Year 2020 und andere Seite 2018 als Lieferung/Kontext
- **Ursprüngliche Nutzung:** unbebautes Grundstück
- **Neue Nutzung:** Einfamilienhaus mit Atelier
- **Fläche / Maßstab:** 255 m² laut FCRBE; 213 m² laut be.exemplary/Architectura-Artikel; Konflikt offen
- **Schutzstatus / Denkmalstatus:** unbekannt
- **Quellenlage:** sehr gut für Reuse-Materialien und Mengen; begrenzt für Prüfung, Kosten, CO₂ und Normen

---

## 4. REUSE-STRATEGIE

- **Art der Wiederverwendung:** partiell; ex-situ; Bauteilwiederverwendung; Materialwiederverwendung; Neubau mit Direct Reuse
- **Hauptniveau:** Gebäudehülle + räumlicher Innenausbau + Sanitär/feste Technik
- **Unterschied zu Sanierung, Recycling oder Bestandserhalt:** Es handelt sich nicht um Bestandserhalt, sondern um Neubau. Ziegel, Fliesen, Platten und Sanitär werden als Bauteile wieder eingebaut, nicht zu Recyclinggranulat verarbeitet.
- **Warum ist der Fall relevant?** Er zeigt, dass in einem kleinen privaten Neubau mehrere Reuse-Lieferketten kombiniert werden können, ohne dass das Haupttragwerk selbst reuse-basiert sein muss.

---

## 5. BAUTEIL-INVENTAR

| Bauteil | Material | Herkunft | alte Funktion | neue Funktion | Menge/Umfang | tragend? | räumlich? | Hülle? | technisch? | Eingriff/Aufbereitung | Verbindung | Prüfung | Leistungsanforderung | Norm/Recht | Hürde | Quelle | unbekannt |
|---|---|---|---|---|---:|---|---|---|---|---|---|---|---|---|---|---|---|
| Fassaden-Claustra-Ziegel | Large/Grote Boomse Steen, Ziegel 205 x 55 x 100 mm | Franck Bricks / lokaler Reuse-Händler | Mauerwerksziegel unbekannt | vordere Fassaden-Claustra | 3.000 Stück / 36 m² | nicht als tragend belegt | ja | ja | nein | Auswahl/Reinigung unbekannt | Mauerwerk; breite Ziegel erlaubten Claustra ohne zusätzliche metallische Aufhängung | unbekannt | Hülle, Dauerhaftigkeit, Ausdruck | unbekannt | Format/Verfügbarkeit | [S2], [S3] | Herkunftsgebäude, Prüfung |
| Restliche Ziegel | gleicher Lot | Baustellenüberschuss | unbekannt | kleine Außentreppe im Garten | Menge unbekannt | unbekannt | ja | nein | nein | Wiederverwendung von Überschuss | Mauerwerk unbekannt | unbekannt | Außenbereich | unbekannt | nicht geplant? | [S2] | Menge |
| Wandfliesen | Stein / geschnittene Platte, aus Solvay-Gebäude | Rotor DC | Wand-/Innenbekleidung | Wandfliesen Sanitär/Innenraum | 21 m² | nein | ja | nein | nein | Demontage/Reinigung unbekannt | Fliesenkleber/Mörtel unbekannt | unbekannt | Feuchte, Reinigung | unbekannt | passende Menge | [S2] | Prüfung |
| Bodenfliesen | Terrakotta | Rotor DC | Bodenbelag | Bodenbelag | 13,5 m² | nein | ja | nein | nein | Reinigung unbekannt | Kleber/Mörtel unbekannt | unbekannt | Abrieb, Reinigung | unbekannt | Chargen-/Maßvarianz | [S2] | Herkunft |
| Blausteinplatten | Pierre bleue / Blaustein | 2emain.be Kleinanzeigen | Boden-/Außenplatten unbekannt | Eingangshalle und Terrasse | 40 m² | nein | ja | teils außen | nein | Sortieren/Zuschneiden unbekannt | Plattenbelag | unbekannt | Außen-/Innenbelag, Dauerhaftigkeit | unbekannt | informelle Beschaffung | [S2] | Herkunft, Prüfung |
| Sanitärobjekte | Keramik/Metall unbekannt | Rotor DC | Sanitär | Bäder/Atelierbecken | Menge nicht vollständig; SDB-Becken, Vasques, Atelierbecken | nein | ja | nein | ja | Reinigung/Funktionsprüfung unbekannt | Sanitäranschlüsse | unbekannt | Hygiene, Dichtheit, Wartung | unbekannt | Hygiene/Kompatibilität | [S2] | genaue Stückzahl |
| Tragstruktur | Holz | neu, lokal/nachhaltig bewirtschaftet laut FCRBE | neu | Haupttragwerk | unbekannt | ja | ja | nein | nein | Neubau | Holzverbindungen unbekannt | unbekannt | Lastabtragung | unbekannt | nicht Direct Reuse | [S2], [S3] | Mengen |
| Strohballen | Stroh | neu | neu | Dämmfüllung Fassaden | unbekannt | nein / teils aussteifend unbekannt | nein | Hülle/Dämmung | nein | neu | Füllung im Holzrahmen | unbekannt | Wärme/Feuchte | unbekannt | nicht Direct Reuse | [S2], [S3] | Kennwerte |
| Hanf-Kalk-Blöcke | Hanf/Kalk | neu | neu | Innenwände | unbekannt | unbekannt | ja | nein | nein | neu | Mauerwerk | unbekannt | Raumklima, Feuchte | unbekannt | nicht Direct Reuse | [S2], [S3] | Mengen |

---

## 6. PROZESS UND LOGISTIK

| Prozessphase | Handlung | Akteure | Methode | Werkzeug/Tool/Software | Abbruchmethode | Aufbereitungsmethode | Prüfung | Logistik | Hürde | Lösung | Quelle |
|---|---|---|---|---|---|---|---|---|---|---|---|
| Bestandsaufnahme | Kein Gebäudebestand; Suche nach Reuse-Bauteilen | Bauherrschaft, Karbon | Materialsuche parallel zum Entwurf | unbekannt | entfällt | unbekannt | unbekannt | mehrere Lieferketten | Verfügbarkeit kleiner Mengen | kleine Projektgröße erlaubt spezifische Lots | [S2], [S3] |
| Bauteilinventar | Auswahl verfügbarer Ziegel, Fliesen, Platten, Sanitär | Karbon, Händler | komponentenbasierte Planung | unbekannt | unbekannt | Sortierung/Reinigung unbekannt | unbekannt | Händler + Kleinanzeigen | Mengen- und Maßbindung | Entwurf an verfügbare Bauteile anpassen | [S2] |
| Schadstoffprüfung | unbekannt | unbekannt | unbekannt | unbekannt | unbekannt | unbekannt | unbekannt | unbekannt | Sanitär/Fliesen-Herkunft | unbekannt | keine Quelle |
| Rückbau | durch vorgelagerte Lieferanten/Quellen | Franck, Rotor DC, unbekannte Vorbesitzer | selektive Demontage unbekannt | unbekannt | unbekannt | unbekannt | unbekannt | ex-situ | Herkunft nicht komplett publiziert | Händler übernehmen Teile der Kette | [S2] |
| Ausbau | unbekannt | Lieferanten | unbekannt | unbekannt | unbekannt | unbekannt | unbekannt | unbekannt | unbekannt | unbekannt | keine Quelle |
| Transport | Lieferung der Reuse-Bauteile zur Baustelle | Lieferanten / Bauunternehmen | Kleinmengenlogistik | unbekannt | unbekannt | unbekannt | unbekannt | Auderghem | Koordination mehrerer Quellen | Projektgröße reduziert Risiko | [S2] |
| Lagerung | unbekannt | Bauunternehmen | unbekannt | unbekannt | unbekannt | unbekannt | unbekannt | unbekannt | Bruch/Sortierung | unbekannt | keine Quelle |
| Aufbereitung | mögliche Reinigung/Zuschnitt/Sortierung | Händler/Ausführende | unbekannt | unbekannt | unbekannt | unbekannt | unbekannt | unbekannt | Qualitätsschwankung | robuste Bauteile wählen | [S2] |
| Planung | Reuse-Ziegel als Claustra; bio-/geobasierte Struktur | Karbon, Ingenieure | Entwurf nach Materialeigenschaften | unbekannt | entfällt | unbekannt | unbekannt | Abstimmung auf verfügbare Maße | Fassadenlösung ohne Metallaufhängung | breiter Ziegeltyp gewählt | [S2] |
| Genehmigung | Bau eines privaten Einfamilienhauses | Bauherrschaft/Behörde | unbekannt | unbekannt | entfällt | unbekannt | unbekannt | unbekannt | unbekannt | unbekannt | keine Quelle |
| Wiedereinbau | Einbau von Ziegeln, Belägen, Sanitär | Gauthier Nagant, 3ALJ Construct | traditionelle Bauprozesse mit Reuse-Bauteilen | unbekannt | entfällt | unbekannt | unbekannt | Baustelle | Passgenauigkeit / Restmengen | Restziegel im Garten weitergenutzt | [S2], [S3] |
| Monitoring | unbekannt | unbekannt | unbekannt | unbekannt | entfällt | unbekannt | unbekannt | unbekannt | unbekannt | unbekannt | keine Quelle |

---

## 7. TECHNIK, LEISTUNG, NORMEN

| Thema | Befund | Leistungsanforderung | Norm/Recht | Prüfung | technische Hürde | Lösung | Quelle |
|---|---|---|---|---|---|---|---|
| Tragwerkssystem | Neues Holztragwerk mit Strohballenfüllung | Lastabtragung, Dämmung | unbekannt | unbekannt | Reuse nicht tragend | Neubau-Tragwerk mit ökologischen Materialien | [S2], [S3] |
| Lastabtragung | Reuse-Ziegel nicht als tragende Hauptstruktur belegt | unbekannt | unbekannt | unbekannt | Bewertung begrenzt | als Hüllen-/Raumelement zählen | [S2] |
| Verbindung | Ziegel-Claustra ohne zusätzliche metallische Aufhängung | Stabilität/Fassade | unbekannt | unbekannt | Claustra aus Reuse-Ziegeln | breiter Large Boomse Steen | [S2] |
| Brandschutz | unbekannt | Wohnhaus | unbekannt | unbekannt | Holz/Stroh/Bauteilreuse | unbekannt | keine Quelle |
| Schallschutz | unbekannt | Wohnhaus | unbekannt | unbekannt | unbekannt | unbekannt | keine Quelle |
| Feuchte | Stroh/Hanfkalk und Sanitärflächen | Feuchteregulierung / Nassräume | unbekannt | unbekannt | Nassräume mit Reuse-Fliesen/Sanitär | robuste keramische/steinene Bauteile | [S2], [S3] |
| Wärmeschutz | Strohballen als Dämmung | thermische Leistung | unbekannt | unbekannt | fixe Strohballenmaße | Holzrahmen + Stroh | [S2], [S3] |
| Wärmebrücken | unbekannt | unbekannt | unbekannt | unbekannt | unbekannt | unbekannt | keine Quelle |
| Luftdichtheit | unbekannt | Wohnhaus | unbekannt | unbekannt | bio-/geobasierte Hülle | unbekannt | keine Quelle |
| TGA-Integration | Reuse-Sanitär eingebaut | Hygiene, Dichtheit, Anschlüsse | unbekannt | unbekannt | Anschluss alter Sanitärobjekte | unbekannt | [S2] |
| Barrierefreiheit | unbekannt | Wohnhaus | unbekannt | unbekannt | unbekannt | unbekannt | keine Quelle |
| Dauerhaftigkeit | Ziegel, Stein, Fliesen gelten als robuste Bauteile | lange Nutzungsdauer | unbekannt | unbekannt | Heterogenität | Auswahl langlebiger Reuse-Materialien | [S2] |
| Wartung | unbekannt | Wohnhaus | unbekannt | unbekannt | unbekannt | unbekannt | keine Quelle |
| Zulassung | unbekannt | Wohnbau | unbekannt | unbekannt | Nachweis alter Bauteile | unbekannt | keine Quelle |
| Haftung | unbekannt | privater Bau | unbekannt | unbekannt | Gewährleistung Reuse-Bauteile | unbekannt | keine Quelle |

---

## 8. KENNWERTE

| Kennwert | Wert | Einheit | Methode/Datenmodell/Software | Bilanzgrenze | Quelle | Vertrauensgrad |
|---|---:|---|---|---|---|---|
| wiederverwendete Ziegel | 3.000 | Stück | Projektangabe | Fassadenclaustra | [S2], [S3] | belegt |
| wiederverwendete Ziegelfläche | 36 | m² | Projektangabe | Fassade | [S2], [S3] | belegt |
| Wandfliesen Reuse | 21 | m² | Projektangabe | Innenausbau | [S2] | belegt |
| Bodenfliesen Reuse | 13,5 | m² | Projektangabe | Boden | [S2] | belegt |
| Blausteinplatten Reuse | 40 | m² | Projektangabe | Eingang/Terrasse | [S2] | belegt |
| Fläche | 255 | m² | FCRBE case sheet | Gebäude | [S3] | belegt |
| Fläche alternative Quelle | 213 | m² | Presse/Be Exemplary | Gebäude | [S5] | teilweise belegt |
| Projektzeitraum | 2018–2020 | Jahre | FCRBE | Projekt | [S3] | belegt |
| CO₂-Einsparung | unbekannt | kg CO₂e | unbekannt | unbekannt | keine Quelle | unbekannt |
| Kosten | unbekannt | € | unbekannt | unbekannt | keine Quelle | unbekannt |
| Transportdistanz | unbekannt | km | unbekannt | unbekannt | keine Quelle | unbekannt |
| U-Wert / Energiebedarf | unbekannt | unbekannt | unbekannt | unbekannt | keine Quelle | unbekannt |

---

## 9. HÜRDEN-MATRIX

| Hürde | Kategorie | Ursache | Auswirkung | betroffene Entitäten | Lösung | übertragbare Lehre | Quelle |
|---|---|---|---|---|---|---|---|
| Reuse vs. bio-basierte Neubauanteile sauber trennen | methodisch | Projekt kombiniert mehrere ökologische Strategien | Gefahr, neue Bio-Materialien als Reuse zu werten | Material, Reuse-Strategie, Bewertung | Nur tatsächliche Reuse-Bauteile zählen | Materialallianzen quellenkritisch entflechten | [S2], [S3] |
| Verfügbarkeit kleiner passender Chargen | logistisch | Reuse-Lots sind begrenzt | Entwurf muss flexibel sein | Bauteil, Logistik | Einfamilienhausmaßstab nutzt Kleinmengen | Kleine Projekte eignen sich für spezifische Lots | [S2] |
| Konstruktive Integration Claustra | technisch/gestalterisch | Reuse-Ziegel haben feste Maße | Zusatzunterkonstruktion vermeiden | Verbindung, Bauteil | Large Boomse Steen erlaubt Claustra ohne Metallaufhängung | Materialeigenschaft kann Detail steuern | [S2] |
| Informelle Beschaffung | wirtschaftlich/logistisch | Blausteinplatten über Kleinanzeigen | Gewährleistung/Prüfung offen | Bauteilbörse, Recht, Prüfung | robuste, einfache Bauteile wählen | Informelle Märkte können funktionieren, brauchen aber Dokumentation | [S2] |
| Fehlende Prüf-/CO₂-Daten | quellenkritisch | Projektquellen liefern Mengen, aber keine Prüfberichte | begrenzte technische Übertragbarkeit | Prüfung, Kennwert, Norm | Datenlücke offen ausweisen | Nicht alle Circularity-Fälle liefern belastbare Kennwerte | keine Quelle |

---

## 10. WIRTSCHAFT UND BESCHAFFUNG

- **Beschaffungsmodell:** privates Projekt mit mehreren Reuse-Lieferquellen.
- **Bauteilbörse / Quelle:** Franck Bricks für Ziegel; Rotor DC für Fliesen/Sanitär; 2emain.be für Blausteinplatten.
- **Kostenwirkung:** unbekannt
- **Zeitwirkung:** unbekannt
- **Versicherung / Haftung:** unbekannt
- **Gewährleistung:** unbekannt
- **Arbeitsaufwand:** vermutlich erhöht durch Materialsuche und Anpassung; nicht quantifiziert.
- **Lagerung:** unbekannt
- **Marktbarrieren:** kleinteilige Verfügbarkeit, Kompatibilität von Maßen, unbekannte Prüf-/Gewährleistungslogik.

---

## 11. GESTALTUNG UND KULTURELLER WERT

- **Sichtbarkeit der Wiederverwendung:** hoch bei der Ziegelfassade/Claustra; mittel bei Fliesen/Stein/Sanitär.
- **Räumliche Transformation:** Neubau um einen Patio mit Verbindung zwischen Straße, Wohnbereich und Garten.
- **Atmosphäre / Ausdruck:** Materialreiche, warme, handwerkliche Hülle; Reuse und bio-/geobasierte Materialien ergänzen sich.
- **Umgang mit Spuren:** Spuren sind vor allem materialästhetisch, nicht museal.
- **Sozialer Wert:** privates Wohnhaus mit Atelier; keine weiter belegte öffentliche Nutzung.
- **Denkmal- oder Bestandswert:** unbekannt
- **Kritik / Grenzen:** Hauptstruktur ist neu; Reuse bleibt partiell und überwiegend Hülle/Ausbau. Keine CO₂-/Kosten-/Prüfdaten gefunden.

---

## 12. OFFENE ENTITÄTEN UND DATENLÜCKEN

- **Welche bestehenden Entitäten wurden nicht gefunden?** Abbruchmethode, Bericht zu Prüfungen, Normen, Schadstoffprüfung, Software, detaillierte Wirtschaft, Versicherung, Gewährleistung, Monitoring.
- **Welche neuen Entitäten wären sinnvoll?** Materialallianz; informeller Reuse-Markt; Kleinmengenlogistik.
- **Welche Daten fehlen?** Herkunftsgebäude der Ziegel, exakte Stückzahl der Sanitärteile, Prüf- und Zulassungsdetails, Kosten, Transportdistanzen, CO₂-Bilanz, U-Werte.
- **Welche Quellen müssten geprüft werden?** Original-Bauakten, Karbon-Ausführungsdetails, Händlerrechnungen, Bruxelles Environnement Reportage, FCRBE-Voll-PDF.

---

## 13. ABSCHLUSS

- **Soll der Fall in die Hauptliste?** Anhang / Vergleichsfall; ja als mittlerer Hüllen-/Ausbau-Reuse-Fall, nein als Hauptfall tragender Wiederverwendung.
- **5 wichtigste Fakten:**
  1. Neubau-Einfamilienhaus in Auderghem.
  2. Entwurf von Karbon’ architecture & urbanisme.
  3. 3.000 wiederverwendete Large/Grote Boomse Steen für 36 m² Fassadenclaustra.
  4. Weitere Reuse-Bauteile: Fliesen, Blausteinplatten, Sanitär.
  5. Haupttragwerk neu aus Holz/Stroh; daher kein Tragwerksreuse-Hauptfall.
- **5 wichtigste Bauteile:**
  1. Ziegel-Fassadenclaustra
  2. Blausteinplatten in Eingang/Terrasse
  3. Wandfliesen aus Solvay-Gebäude
  4. Terrakotta-Bodenfliesen
  5. Sanitärbecken/-vasen/-Atelierbecken
- **5 wichtigste Hürden:**
  1. Materialverfügbarkeit
  2. Format-/Detailintegration der Ziegel
  3. fehlende Prüf-/Normdaten
  4. informelle Beschaffungswege
  5. Abgrenzung Reuse vs. neue bio-basierte Materialien
- **5 wichtigste übertragbare Erkenntnisse:**
  1. Kleine Neubauten können mehrere Reuse-Kanäle kombinieren.
  2. Reuse muss nicht tragend sein, um architektonisch relevant zu sein.
  3. Robuste Bauteile wie Ziegel, Stein, Fliesen und Sanitär sind besonders geeignet.
  4. Entwurfsdetails sollten aus den verfügbaren Bauteilmaßen entstehen.
  5. Materialmixe brauchen klare Bewertungsgrenzen.
- **5 offene Fragen:**
  1. Welche Prüfungen wurden an Reuse-Ziegeln/Fliesen/Sanitär durchgeführt?
  2. Welche Kostenwirkung hatte die Wiederverwendung?
  3. Wie weit wurden die Bauteile transportiert?
  4. Gibt es eine CO₂-Bilanz?
  5. Welche Gewährleistung wurde für Reuse-Bauteile vereinbart?

---

## Quellen und Links

- [S1] Interne Ausgangsliste: `gebäude4_wiederverwendung_direct_reuse_examples.md`, Priorität 54.
- [S2] Opalis – Maison Vignette: https://opalis.eu/fr/projets/maison-vignette
- [S3] FCRBE / Bruxelles Environnement – 32 detailed project sheets, #07 Maison Vignette: https://gidsduurzamegebouwen.brussels/sites/default/files/documents/2024-06/dt4_2_2_project-sheets_041023_lr.pdf
- [S4] Opalis – Maison Vignette, Karbon’ architecture et urbanisme: https://opalis.eu/fr/projets/maison-vignette-karbon-architecture-et-urbanisme
- [S5] Architectura.be – Be.Exemplary 3, Projekt Vignette: https://www.architectura.be/fr/actualite/voici-les-laureats-de-l-appel-a-projets-beexemplary-3/
