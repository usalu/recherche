---
id: "Chiro_d_Itterbeek_Dilbeek"
entity: "fallstudie"
node_kind: "core"
migration_status: "migrated_phase4_case_graph"
title: "Chiro d’Itterbeek / Sanitary block, Dilbeek – Fallstudie Direct Reuse"
bauobjekt:
  - "Chiro_d_Itterbeek_Dilbeek"
legacy_paths:
  - "Gebäude\\Chiro_d_Itterbeek_Dilbeek.md"
projekt:
  - "Chiro_d_Itterbeek_Dilbeek"
reuse_chain_detected: "False"
---
# Chiro d’Itterbeek / Sanitary block, Dilbeek – Fallstudie Direct Reuse

## Migration

- Fallstudie ID: Chiro_d_Itterbeek_Dilbeek
- Legacy source count: 1
- Generated project: Chiro_d_Itterbeek_Dilbeek
- Generated bauobjekt: Chiro_d_Itterbeek_Dilbeek
- Extracted reuse_einsatz rows: 13
- Extracted datenpunkt rows: 20
- Extracted entity mapping rows: 21
- Reuse chain detected: False

## Legacy Content

### Legacy Source: Gebäude\Chiro_d_Itterbeek_Dilbeek.md

- Map action: split_into_case_graph
- Primary target: fallstudie/Chiro_d_Itterbeek_Dilbeek
- Secondary targets: projekt/Chiro_d_Itterbeek_Dilbeek; bauobjekt/<from_content>; reuse_einsatz/<per_component>
- Risk flags: do_not_treat_file_as_single_gebaeude_only

# Chiro d’Itterbeek / Sanitary block, Dilbeek – Fallstudie Direct Reuse

**Recherchezeitpunkt:** 2026-05-06  
**Sprache:** Deutsch  
**Grundregel angewandt:** Gezählt werden feste Bau-, Hüll-, Technik- und Innenausbauteile. Lose Möbel werden nicht bewertet. Bei diesem Fall zählen u. a. Fassadenziegel, Fenster/Türen, Fliesen, Sanitär, Leuchten und feste technische Bauteile; bloße „surplus“-Neumaterialien werden separat markiert.

---

## 1. EINORDNUNG

- **Entscheidung:** VERGLEICHSFALL / kleiner Demonstrator
- **Bewertung:** ★★★☆☆
- **Begründung:** Sehr gut dokumentierter kleiner Neubau/Sanitärpavillon mit vielen wiederverwendeten und Restposten-Bauteilen in Fassade, Dach, Ausbau, Sanitär und Technik. Tragende Bauteile stammen überwiegend aus Restposten/Produktionsüberschüssen und sind nicht eigentliche Direct-Reuse-Bauteile; Maßstab nur 15 m².
- **Vertrauensgrad:** belegt
- **Warnung Bestandserhalt:** nein – Anbau an bestehende Farm, aber Pavillon ist neu.
- **Warnung Möbel/Dekoration:** ja – 2 PMR-Bars und Wickeltisch/Mobiliar wurden berichtet, werden für die Reuse-Bewertung nicht gewertet, wenn nicht fest konstruktiv relevant.
- **Projektstatus:** gebaut

---

## 2. ENTITÄTEN-MAPPING

| Entität | Wert | Beziehung zur Fallstudie | Quelle/Beleg | Vertrauensgrad | Anmerkung |
|---|---|---|---|---|---|
| Fallstudie | Sanitary block for the Itterbeek Chiro / Pavillon de sanitaires | Untersuchter Fall | [S1], [S2], [S3], [S4] | belegt | Ausgangsliste hatte „verify“; Quellen bestätigen Reuse umfassend. |
| Gebäude | Sanitärpavillon neben alter Farm | Zielgebäude | [S2], [S3] | belegt | Kleine Erweiterung für Jugendorganisation Chiro. |
| Projekt | Neubau/Extension + Renovation-Kontext | Projektart | [S2] | belegt | 2018 Baujahr, 2019 Lieferung. |
| Ort | Plankenstraat 23, 1701 Dilbeek, Belgien | Standort | [S2] | belegt | Itterbeek / Dilbeek. |
| People | Rotor asbl-vzw | Entwurf / Design-Build | [S2], [S3], [S4] | belegt | Designer-contractor laut Construction21. |
| Bauherr | Commune de Dilbeek | Eigentümer Sanitärblock / Auftraggeber | [S2], [S4] | belegt | Farm selbst Eigentum kirchlicher Verwaltung. |
| People | CC Autrement | Konstrukteur / Ausführung | [S2], [S4] | belegt | Partner im Design-Build. |
| Bauteilbörse | Franck, RotorDC, Gebruiktebouwmaterialen, Namur Croisade pauvreté, Bouwstocks, kleine Anzeigen | Lieferquellen | [S2], [S4], [S5], [S6] | belegt | Mischung Händler, Spende, Restposten, Kleinanzeigen. |
| Bauteil | Fassadenziegel | Reuse-Hülle | [S2], [S3], [S4] | belegt | 30 m². |
| Bauteil | Stahl-U-Profile als Außenstürze | Reuse-Konstruktion/Fassade | [S2], [S4] | belegt | 5 lfm / 2 U-Profile. |
| Bauteil | Dachziegel | Reuse/Restposten Dach | [S2], [S4] | belegt | 20 m², Rest einer Brüsseler Villa-Renovierung. |
| Bauteil | Außentüren und Holzfenster | Reuse-Hülle | [S2], [S4] | belegt | 2 Türen, 2 Holzfenster. |
| Bauteil | Boden- und Wandfliesen | Reuse-Innenausbau | [S2], [S4] | belegt | 14 m² Boden / 11 m² Wand. |
| Bauteil | Sanitär | Reuse-Technik | [S2], [S4] | belegt | 4 WCs, 2 Urinale, 3 Art-déco-Waschbecken, Urinaltrenner. |
| Bauteil | Leuchten | Reuse-Technik | [S2], [S4] | belegt | 5 Leuchten. |
| Material | Dämmung, Betonblöcke, Holzrahmen | Restposten / Produktionsüberschüsse | [S2], [S4] | belegt | Nicht immer Direct Reuse; als Surplus dokumentieren. |
| Kennwert | weniger als ein Drittel neue Materialien nach Masse | zirkulärer Gesamtanteil | [S2], [S3] | belegt | Wichtig, aber Mischung aus Reuse und Surplus. |
| Kennwert | 4.572,702786 kg CO₂ vermieden | Umweltkennwert | [S2], [S4] | belegt | Construction21-Rechner; Bilanzgrenze beachten. |
| Prozessphase | alle klassischen Neubauphasen durchgeführt | Übertragbarkeit | [S2], [S3] | belegt | Fundamente, Kanalisation, Rohbau, Sanitär, Elektro, Innenausbau. |
| Hürde | logistisch-organisatorische Hindernisse | Ziel des Projekts | [S2], [S3] | belegt | Pilot/learning experience. |
| Wirtschaft | 55.000 € Bau-/Renovierungskosten; 15 m² | Kennwert | [S2] | belegt | Kosten/m² angegeben. |

### Vorgeschlagene neue Entität

| Neue Entität | Warum nötig? | Beispiel aus dem Fall | Beziehung zu bestehenden Entitäten |
|---|---|---|---|
| Surplus-/Restpostenmaterial | Viele eingesetzte Bauteile sind keine gebrauchten Bauteile, sondern Baustellen-/Produktionsüberschüsse. | Dämmung, Betonblöcke, Holzrahmen aus Restposten/End-of-stock. | Ergänzt Material, Bauteilbörse, Wirtschaft, Reuse-Strategie. |
| Design-Build-Reuse | Entwurf und Ausführung liegen bei einer reuse-erfahrenen Organisation; wichtig für Haftung/Koordination. | Rotor als concepteur-contractant mit CC Autrement. | Verknüpft Projekt, Prozessphase, Wirtschaft und Hürde. |

---

## 3. FALLSTUDIE

- **Name:** Chiro d’Itterbeek / Sanitary block for the Itterbeek Chiro / Pavillon de sanitaires du Chiro d’Itterbeek
- **Ort:** Plankenstraat 23, 1701 Dilbeek, Belgien
- **Gebäude:** kleiner Sanitärpavillon neben alter Farm
- **Projekt:** Neubau/Anbau eines Sanitärblocks für Chiro d’Itterbeek und Festival-/Besucherbedarf
- **Beteiligte People / Akteure:** Commune de Dilbeek; Rotor asbl-vzw; CC Autrement; RotorDC; Franck; Gebruiktebouwmaterialen; Namur Croisade pauvreté; Bouwstocks
- **Architekt:** Rotor asbl-vzw
- **Tragwerksplaner:** unbekannt
- **Bauherr:** Commune de Dilbeek; Farm im Eigentum der kirchlichen Verwaltung von Itterbeek
- **Zeitraum:** Baujahr 2018; Lieferung 2019
- **Ursprüngliche Nutzung:** neben bestehender Farm; kein vorheriger Sanitärpavillon belegt
- **Neue Nutzung:** Sanitärblock / WC-Anlage für Chiro und Besucher
- **Fläche / Maßstab:** 15 m² Nettofläche; 1 funktionale Einheit
- **Schutzstatus / Denkmalstatus:** unbekannt
- **Quellenlage:** sehr gut für Mengen, Lieferquellen, Kosten, Umweltkennwerte; begrenzt für Norm-/Prüfdetails und konkrete Tragwerksnachweise

---

## 4. REUSE-STRATEGIE

- **Art der Wiederverwendung:** partiell; ex-situ; Bauteilwiederverwendung; Materialwiederverwendung; Technik-/Sanitärreuse; Hüllenreuse; ergänzt durch Restposten-/Surplus-Materialien
- **Hauptniveau:** Gebäudehülle + räumlicher Innenausbau + technische Gebäudeausrüstung + Material
- **Unterschied zu Sanierung, Recycling oder Bestandserhalt:** Der Sanitärblock ist neu, aber verwendet wiederverwendete Bauteile und Restposten. Reuse ist nicht Recycling zu Granulat; Bauteile bleiben als Ziegel, Fliesen, Fenster, Sanitär, Leuchten usw. erhalten.
- **Warum ist der Fall relevant?** Trotz nur 15 m² zeigt der Fall sehr konkret, welche Bauteilgruppen in einem klassischen Neubauprozess mit Reuse/Surplus umgesetzt werden können und welche logistischen Hürden auftreten.

---

## 5. BAUTEIL-INVENTAR

| Bauteil | Material | Herkunft | alte Funktion | neue Funktion | Menge/Umfang | tragend? | räumlich? | Hülle? | technisch? | Eingriff/Aufbereitung | Verbindung | Prüfung | Leistungsanforderung | Norm/Recht | Hürde | Quelle | unbekannt |
|---|---|---|---|---|---:|---|---|---|---|---|---|---|---|---|---|---|---|
| Fassadenziegel | Ziegel, 3 Typen: gelber industrieller Ziegel + Feldbrandziegel klassisch/dünn | Franck / Opalis-Händler | Fassaden-/Mauerwerksziegel unbekannt | Fassadenbekleidung / äußeres Mauerwerk | 30 m²; Adokin auch 2,7 m³ | nicht als tragend belegt | ja | ja | nein | Reinigen/Sortieren nicht detailliert | Mauerwerk in Schichten | unbekannt | Witterung, Integration in Backsteinumfeld | unbekannt | passende Menge/Typen | [S2], [S3], [S4], [S7] | Herkunftsgebäude, Prüfung |
| Außenstürze | Stahl-U-Profile | Gebruiktebouwmaterialen / Opalis-Händler | Stahlprofile unbekannt | Fassadenstürze | 5 lfm; 2 U-Profile | ja, lokal als Sturz | ja | ja | nein | zwei U-Profile zusammengefügt | verschraubt/verschweißt unbekannt | unbekannt | Lastabtragung über Öffnungen | unbekannt | Verbindung/Nachweis | [S2], [S4] | Details |
| Dachziegel | Dachziegel | Rest einer Brüsseler Villenrenovierung / Spende | Dachdeckung | Dachdeckung | 20 m² | nein | nein | ja | nein | Sortieren | Dachdeckung | unbekannt | Witterung | unbekannt | Anfangsmenge war laut Quelle falsch; Nachsuche nötig | [S2], [S4] | genaue Herkunft |
| Außentüren | Holz | Namur Croisade pauvreté / Gebruiktebouwmaterialen | Türen | Außentüren | 2 Stück | nein | ja | ja | nein | Anpassung unbekannt | Türbänder/Rahmen | unbekannt | Zugang, Sicherheit, Wetter | unbekannt | Maße/Kompatibilität | [S2], [S4] | Prüfung |
| Holzfenster | Holz/Glas | Namur Croisade pauvreté / Gebruiktebouwmaterialen | Fenster | Außenfenster | 2 Stück | nein | ja | ja | nein | Anpassung unbekannt | Rahmen | unbekannt | Licht, Luft, Witterung | unbekannt | Maße/U-Wert unbekannt | [S2], [S4] | U-Wert |
| Bodenfliesen | Keramik, aus ehemaliger Grundschule Everheide, 1930er | RotorDC | Bodenbelag | Sanitärboden | 14 m² | nein | ja | nein | nein | Reinigung | Fliesenbelag | unbekannt | Abrieb, Reinigung, Hygiene | unbekannt | Menge/Bruch | [S2], [S4] | Prüfung |
| Wandfliesen | Keramik | RotorDC | Wandbekleidung | Nassbereich Wand | 11 m² | nein | ja | nein | nein | Reinigung | Fliesenbelag | unbekannt | Wasser/Reinigung | unbekannt | Menge/Passung | [S2], [S4] | Herkunft |
| Sanitärobjekte | Keramik/Metall | RotorDC | Sanitär | WC/Urinale/Waschbecken | 4 WCs, 2 Duravit-Urinale, 3 Art-déco-Waschbecken, 1 Urinaltrenner | nein | ja | nein | ja | Reinigung/Funktionsprüfung unbekannt | Sanitäranschlüsse | unbekannt | Hygiene, Dichtheit, Wartung | unbekannt | Anschluss/Kompatibilität | [S2], [S4] | Prüfdetails |
| Leuchten | unbekannt | RotorDC | Beleuchtung | Beleuchtung | 5 Stück | nein | nein | nein | ja | unbekannt | Elektroanschluss | unbekannt | elektrische Sicherheit, Beleuchtung | unbekannt | Zulassung/Prüfung | [S2], [S4] | Prüfung |
| Dämmung Boden/Wand | Dämmmaterial | Surplus/End-of-stock/B-Qualität via Gebruiktebouwmaterialen/Bouwstocks | unbenutzt / Restposten | Dämmung | 43,3 m² | nein | nein | Hülle | nein | neu/restposten | Einbau unbekannt | unbekannt | Wärmeschutz | unbekannt | nicht Direct Reuse, aber Abfallvermeidung | [S2], [S4] | Materialtyp |
| Dämmung Decke | Dämmmaterial | Bouwstocks Surplus | unbenutzt / Restposten | Deckendämmung | 18 m² | nein | nein | Hülle | nein | neu/restposten | Einbau unbekannt | unbekannt | Wärmeschutz | unbekannt | nicht Direct Reuse | [S2], [S4] | Materialtyp |
| Betonblöcke | Beton | Bouwstocks Surplus/B-Qualität | unbenutzt / Restposten | Rohbau/Wand | 48 m²; Adokin 6,7 m³ | evtl. ja | ja | ja | nein | neu/restposten | Mauerwerk | unbekannt | Last/Wand | unbekannt | nicht gebrauchte Reuse-Bauteile | [S2], [S4], [S7] | Prüfung |
| Holzrahmen / Dachtragwerk | Holz | Bouwstocks Surplus/Restposten | unbenutzt / Restposten | Dach-/Tragstruktur | 49,2 lfm | ja | ja | teils | nein | neu/restposten | Holzverbindungen | unbekannt | Lastabtragung | unbekannt | Surplus statt Direct Reuse | [S2], [S4] | Dimensionen |

---

## 6. PROZESS UND LOGISTIK

| Prozessphase | Handlung | Akteure | Methode | Werkzeug/Tool/Software | Abbruchmethode | Aufbereitungsmethode | Prüfung | Logistik | Hürde | Lösung | Quelle |
|---|---|---|---|---|---|---|---|---|---|---|---|
| Bestandsaufnahme | Bedarf Sanitärblock + bestehender Farmkontext | Commune de Dilbeek, Rotor | Entwurfs-/Bedarfsanalyse | unbekannt | entfällt | entfällt | unbekannt | Standortanalyse | Integration in Backsteinumfeld | Fassadenmaterial Backstein | [S2], [S3] |
| Bauteilinventar | möglichst viele Reuse-/Surplus-Bauteile identifiziert | Rotor, Händler | Materialsuche nach Baulos | Opalis-Händlernetz / Marketplace | unbekannt | Sortierung/Reinigung | unbekannt | viele kleine Quellen | Koordination vieler Quellen | Design-Build mit reuse-erfahrenem Akteur | [S2], [S4] |
| Schadstoffprüfung | unbekannt | unbekannt | unbekannt | unbekannt | unbekannt | unbekannt | unbekannt | unbekannt | Sanitär/Fliesen/Leuchten gebraucht | unbekannt | keine Quelle |
| Rückbau | vorgelagert bei Lieferanten/Herkunftsorten | Händler | selektive Rückgewinnung unbekannt | unbekannt | unbekannt | unbekannt | unbekannt | ex-situ | unbekannt | Nutzung etablierter Händler | [S2], [S4] |
| Ausbau | Entnahme von Ziegeln, Fliesen, Sanitär, Türen/Fenstern | Händler/Lieferanten | unbekannt | unbekannt | unbekannt | unbekannt | unbekannt | unbekannt | Bruch/Kompatibilität | robuste Bauteile | [S2], [S4] |
| Transport | Lieferung mehrerer kleiner Lots | Rotor, Händler, Ausführende | Kleinmengenlogistik | unbekannt | entfällt | unbekannt | unbekannt | meist Region Brüssel laut Quelle | Transportaufwand beachten | regionale Quellen | [S2], [S4] |
| Lagerung | kurzzeitige Baustellen-/Materiallagerung | Rotor/CC Autrement | unbekannt | unbekannt | entfällt | unbekannt | unbekannt | Baustelle | Sortierung, Bruch | unbekannt | keine Quelle |
| Aufbereitung | Reinigung/Anpassung | Händler/CC Autrement | unbekannt | unbekannt | entfällt | unbekannt | unbekannt | unbekannt | Zeit/Arbeitsaufwand | Händler und kleine Chargen | [S2], [S4] |
| Planung | Entwurf auf verfügbare Komponenten abgestimmt | Rotor | Design-Build / reuse-led design | unbekannt | entfällt | Auswahl | unbekannt | Beschaffung parallel zu Planung | Bauteilverfügbarkeit | Materialmix aus Reuse, Surplus, neu | [S2], [S3] |
| Genehmigung | kommunales Bauprojekt | Commune de Dilbeek | unbekannt | unbekannt | entfällt | unbekannt | unbekannt | unbekannt | öffentliches kleines Projekt | Design-Build-Auftrag | [S2] |
| Wiedereinbau | Umsetzung aller Gewerke | Rotor, CC Autrement | klassischer Neubauprozess mit Reuse/Surplus | unbekannt | entfällt | Bauteile eingebaut | unbekannt | Baustelle | Passung, fehlende Dachziegel | zusätzliche Suche; keine monetäre Einbuße laut Quelle | [S2], [S4] |
| Monitoring | Umweltkennwerte berechnet / Nutzungsenergie nicht präzise | Construction21/Rotor | Impact-Rechner / Erfahrungsbericht | unbekannt | entfällt | entfällt | unbekannt | nach Projekt | Energieverbrauch unbekannt | Nutzung: Wochenende, geringe Verbraucher beschrieben | [S2], [S4] |

---

## 7. TECHNIK, LEISTUNG, NORMEN

| Thema | Befund | Leistungsanforderung | Norm/Recht | Prüfung | technische Hürde | Lösung | Quelle |
|---|---|---|---|---|---|---|---|
| Tragwerkssystem | Betonblöcke und Holzrahmen/Charpente aus Surplus/End-of-stock, nicht hauptsächlich gebraucht | Lastabtragung | unbekannt | unbekannt | nicht klassisches Direct Reuse | als Surplus getrennt dokumentieren | [S2], [S4] |
| Lastabtragung | Stahl-U-Profile als wiederverwendete Stürze | Öffnungsüberdeckung | unbekannt | unbekannt | Verbindung/Nachweis gebrauchter Profile | zwei U-Profile zu Sturz zusammengesetzt | [S2], [S4] |
| Verbindung | Mauerwerk, Fliesen, Sanitäranschlüsse, Elektroanschlüsse | Funktionsfähigkeit | unbekannt | unbekannt | Passung heterogener Bauteile | kleine, robuste Bauteile | [S2], [S4] |
| Brandschutz | unbekannt | Sanitärbau | unbekannt | unbekannt | unbekannt | unbekannt | keine Quelle |
| Schallschutz | unbekannt | kleines Sanitärgebäude | unbekannt | unbekannt | unbekannt | unbekannt | keine Quelle |
| Feuchte | Nassräume mit Reuse-Fliesen und Sanitär | Reinigung/Wasserbeständigkeit | unbekannt | unbekannt | Hygiene/Dichtheit alter Sanitärteile | keramische Flächen nur in Spritzwasserzonen, Wände nicht voll verputzt | [S4] |
| Wärmeschutz | Dämmung aus Surplus/Restposten | Dämmleistung | unbekannt | unbekannt | Materialtyp/Leistung nicht veröffentlicht | Surplus-Dämmung Boden/Wand/Decke | [S2], [S4] |
| Wärmebrücken | unbekannt | unbekannt | unbekannt | unbekannt | unbekannt | unbekannt | keine Quelle |
| Luftdichtheit | unbekannt | kleines Gebäude | unbekannt | unbekannt | unbekannt | unbekannt | keine Quelle |
| TGA-Integration | einfache Lüftung, Regenwasserpumpe, Reuse-Sanitär, Leuchten | Sanitäre Funktion, Beleuchtung | unbekannt | unbekannt | Anschluss gebrauchter Sanitärobjekte | einfache Systeme; kein Warmwasser/Heizung/Kühlung | [S2], [S4] |
| Barrierefreiheit | PMR-Bars erwähnt; WC für PMR | Zugänglichkeit | unbekannt | unbekannt | Wickeltisch/PMR-Bars zählen nicht als zentrale Reuse | feste Ausstattungen eingebaut | [S4] |
| Dauerhaftigkeit | robuste Materialien: Ziegel, Dachziegel, Holzfenster, dicke Keramikfliesen, Sanitär | Nutzung/Wartung | unbekannt | unbekannt | Wiederverwendbarkeit in Zukunft | außer gegossener Platte/Chape und Hohlbetonblöcken sollen Elemente demontierbar sein | [S4] |
| Wartung | Kein Heizen/WW/Kühlen; einfache Nutzung | geringer Betrieb | unbekannt | unbekannt | Nutzungsdaten nicht präzise | Nutzung v. a. Wochenende | [S2], [S4] |
| Zulassung | unbekannt | öffentlich/kommunal | unbekannt | unbekannt | gebrauchte Bauteile | unbekannt | keine Quelle |
| Haftung | Design-Build Rotor/CC Autrement | Verantwortlichkeiten integriert | unbekannt | unbekannt | Reuse im kleinen öffentlichen Auftrag | Designer-contractor-Modell | [S2] |

---

## 8. KENNWERTE

| Kennwert | Wert | Einheit | Methode/Datenmodell/Software | Bilanzgrenze | Quelle | Vertrauensgrad |
|---|---:|---|---|---|---|---|
| Nettofläche | 15 | m² | Construction21 | Gebäude | [S2] | belegt |
| Kosten Bau/Renovierung | 55.000 | € | Construction21 | Projekt | [S2] | belegt |
| Kosten/m² | 3.666,67 | €/m² | Construction21 | Projekt | [S2] | belegt |
| neue Materialien | weniger als 1/3 | % Masse | Projektangabe | gesamte Materialmasse | [S2], [S3] | belegt |
| Fassadenziegel Reuse | 30 | m² | Projektangabe | Fassade | [S2], [S4] | belegt |
| Außenstürze Reuse | 5 | lfm | Projektangabe | Fassade | [S2], [S4] | belegt |
| Dachziegel Reuse/Rest | 20 | m² | Projektangabe | Dach | [S2], [S4] | belegt |
| Außentüren/Fenster | 2 + 2 | Stück | Projektangabe | Hülle | [S2], [S4] | belegt |
| Bodenfliesen Reuse | 14 | m² | Projektangabe | Boden | [S2], [S4] | belegt |
| Wandfliesen Reuse | 11 | m² | Projektangabe | Wand | [S2], [S4] | belegt |
| Sanitär Reuse | 4 WCs, 2 Urinale, 3 Waschbecken, 1 Trenner | Stück | Projektangabe | Sanitär | [S2], [S4] | belegt |
| Leuchten Reuse | 5 | Stück | Projektangabe | Elektro/Beleuchtung | [S2], [S4] | belegt |
| Dämmung Boden/Wand Surplus | 43,3 | m² | Projektangabe | Dämmung | [S2], [S4] | belegt |
| Dämmung Decke Surplus | 18 | m² | Projektangabe | Dämmung | [S2], [S4] | belegt |
| Betonblöcke Surplus | 48 | m² | Projektangabe | Rohbau | [S2], [S4] | belegt |
| Holzrahmen Surplus | 49,2 | lfm | Projektangabe | Charpente | [S2], [S4] | belegt |
| vermiedenes CO₂ | 4.572,702786 | kg CO₂ | Construction21-Impactberechnung; PMR-Bars/Wickeltisch ausgenommen | Reuse-/Surplus-Materialeinsatz | [S2], [S4] | belegt |
| vermiedener Wasserverbrauch | 122,8623915 | m³ | Construction21-Impactberechnung | Reuse-/Surplus-Materialeinsatz | [S2], [S4] | belegt |
| vermiedene Abfälle | 5.400,626347 | kg | Construction21-Impactberechnung | Reuse-/Surplus-Materialeinsatz | [S2], [S4] | belegt |
| Energiebedarf Betrieb | unbekannt | kWh/m²a | Quelle sagt keine präzisen Informationen | Betrieb | [S2], [S4] | unbekannt |

---

## 9. HÜRDEN-MATRIX

| Hürde | Kategorie | Ursache | Auswirkung | betroffene Entitäten | Lösung | übertragbare Lehre | Quelle |
|---|---|---|---|---|---|---|---|
| Viele kleine Lieferquellen | logistisch | Reuse/Surplus über Händler, Spende, Kleinanzeigen | Koordinationsaufwand | Logistik, Bauteilbörse, Wirtschaft | Rotor als Designer-Contractor; reuse-erfahrene Händler | Reuse braucht Beschaffungskompetenz früh im Projekt | [S2], [S4] |
| Mengenangabe Spender falsch | logistisch/wirtschaftlich | Dachziegel-Spende reichte nicht wie erwartet | Zeitverlust für fehlende Dachziegel | Bauteil, Logistik | Nachsuche; laut Quelle kein Geldverlust auf diesem Posten | kostenlose Materialien haben Transportrisiko und Mengenrisiko | [S4] |
| Abgrenzung Reuse vs. Surplus | methodisch | viele Bauteile sind Restposten, nicht gebraucht | Bewertungsrisiko | Material, Bewertung | getrennt als Reuse bzw. Surplus dokumentieren | Nicht alles Zirkuläre ist Direct Reuse | [S2], [S4] |
| Prüf-/Zulassungsdetails unbekannt | rechtlich/technisch | gebrauchte Sanitär-/Elektro-/Fensterteile | Übertragbarkeit begrenzt | Prüfung, Norm, Haftung | robuste, einfache Systeme; Design-Build | Kleinmaßstab senkt Risiko, ersetzt aber keine Dokumentation | keine Quelle |
| Nutzungsenergie unbekannt | ökologisch | seltene Wochenendnutzung, keine präzisen Daten | keine vollständige Betriebsbewertung | Kennwert, Monitoring | Betrieb qualitativ beschrieben | Materialimpact und Betrieb getrennt bilanzieren | [S2], [S4] |

---

## 10. WIRTSCHAFT UND BESCHAFFUNG

- **Beschaffungsmodell:** kommunaler Auftrag; Design-Build/Designer-Contractor Rotor mit CC Autrement.
- **Bauteilbörse / Quelle:** Franck für Ziegel; Gebruiktebouwmaterialen für Stahlprofile/Dämmung; RotorDC für Fliesen, Sanitär, Leuchten; Namur Croisade pauvreté für Türen/Fenster; Bouwstocks für Surplus-Dämmung/Betonblöcke/Holz; private Brüsseler Baustelle/Villa-Renovierung als Spende für Dachziegel.
- **Kostenwirkung:** Gesamt 55.000 € / 3.666,67 €/m² belegt; spezifische Mehr-/Minderkosten durch Reuse unbekannt. Dachziegellos: Quelle nennt keinen Geldverlust, aber Zeitverlust/Nachsuche.
- **Zeitwirkung:** kleine Zeitverluste bei fehlenden Dachziegeln belegt; sonst unbekannt.
- **Versicherung / Haftung:** unbekannt; Design-Build könnte Koordination vereinfachen, ist aber kein belegter Haftungsnachweis.
- **Gewährleistung:** unbekannt
- **Arbeitsaufwand:** erhöht durch Suche, Koordination, Anpassung und Wiedereinbau; nicht quantifiziert.
- **Lagerung:** unbekannt
- **Marktbarrieren:** Verfügbarkeit, Mengenunsicherheit, Prüfnachweise, elektrische/sanitäre Kompatibilität, Transportkosten auch bei Gratisware.

---

## 11. GESTALTUNG UND KULTURELLER WERT

- **Sichtbarkeit der Wiederverwendung:** hoch in der geschichteten Backsteinfassade; mittel in Fliesen/Sanitär.
- **Räumliche Transformation:** kleiner Neubau ergänzt die bestehende Farm und schafft eine nutzbare Sanitärinfrastruktur.
- **Atmosphäre / Ausdruck:** bewusst in Backsteinumgebung integriert, aber mit atypischem Schichtmauerwerk eigenständig.
- **Umgang mit Spuren:** alte rote/gelbe Ziegel und Keramikfliesen zeigen Materialgeschichte, ohne museale Überhöhung.
- **Sozialer Wert:** Infrastruktur für Jugendorganisation Chiro und Besucher/Festivalnutzung.
- **Denkmal- oder Bestandswert:** alter Farmkontext, Schutzstatus unbekannt.
- **Kritik / Grenzen:** kleiner Maßstab; mehrere Bauteile sind Surplus statt Direct Reuse; Norm-/Prüfdetails fehlen.

---

## 12. OFFENE ENTITÄTEN UND DATENLÜCKEN

- **Welche bestehenden Entitäten wurden nicht gefunden?** Normnummern, detaillierte Prüfung, Schadstoffprüfung, Tragwerksplanung, Software/Datenmodell, Versicherung/Gewährleistung, Lagerung im Detail.
- **Welche neuen Entitäten wären sinnvoll?** Surplus-/Restpostenmaterial; Design-Build-Reuse; Kleinmengenlogistik; Gratis-/Spendenmaterial.
- **Welche Daten fehlen?** genaue Prüfungen von Leuchten/Sanitär/Fenstern/Stahlprofilen, Gewährleistungsmodell, exakte Transportdistanzen, Betriebsmessdaten, Lebensdauerannahmen.
- **Welche Quellen müssten geprüft werden?** Rotor-Projektdossier, Ausführungs-/Prüfunterlagen, Rechnungen/Liefernachweise, kommunale Ausschreibung, technische Details der Stürze und Anschlüsse.

---

## 13. ABSCHLUSS

- **Soll der Fall in die Hauptliste?** Anhang / Vergleichsfall; ja als sehr gut dokumentierter kleiner Demonstrator, nein als Hauptfall wegen Maßstab und begrenzter tragender Direct Reuse.
- **5 wichtigste Fakten:**
  1. 15 m² Sanitärpavillon in Dilbeek/Itterbeek, geliefert 2019.
  2. Rotor entwarf und realisierte mit CC Autrement ein Reuse-/Surplus-Demonstratorprojekt.
  3. Weniger als ein Drittel der Materialien sind neu nach Masse.
  4. Fassade vollständig aus Reuse-Ziegeln und Reuse-Schreinerei laut Construction21/Rotor.
  5. Construction21 nennt 4.572,702786 kg CO₂, 122,8623915 m³ Wasser und 5.400,626347 kg Abfall vermieden.
- **5 wichtigste Bauteile:**
  1. 30 m² Fassadenziegel
  2. 5 lfm Stahl-U-Profil-Stürze
  3. 20 m² Dachziegel
  4. 14 m² Bodenfliesen + 11 m² Wandfliesen
  5. Sanitärteile: 4 WCs, 2 Urinale, 3 Waschbecken, Urinaltrenner
- **5 wichtigste Hürden:**
  1. Koordination vieler kleiner Lieferquellen
  2. falsche Mengenangabe beim Dachziegelspender
  3. Abgrenzung Reuse vs. Surplus
  4. unbekannte Prüf-/Zulassungsdetails
  5. Transport- und Zeitaufwand auch bei günstigen/geschenkten Bauteilen
- **5 wichtigste übertragbare Erkenntnisse:**
  1. Kleine öffentliche Gebäude eignen sich als Lernlabore für Reuse.
  2. Design-Build kann Reuse-Koordination vereinfachen.
  3. Feste Sanitär- und Elektrobauteile können zählen, wenn sie wieder eingebaut werden.
  4. Surplus-Material sollte separat von echter Bauteilwiederverwendung bilanziert werden.
  5. Robuste, einfache Bauteile reduzieren technische Integrationsrisiken.
- **5 offene Fragen:**
  1. Welche Prüfungen wurden für gebrauchte Leuchten, Sanitär und Stahlprofile durchgeführt?
  2. Wie wurden Gewährleistung und Haftung geregelt?
  3. Welche Transportdistanzen entstanden tatsächlich?
  4. Welche Materialanteile sind echte Reuse vs. Surplus in kg?
  5. Gibt es Messdaten zum Betrieb nach Fertigstellung?

---

## Quellen und Links

- [S1] Interne Ausgangsliste: `gebäude4_wiederverwendung_direct_reuse_examples.md`, Priorität 56.
- [S2] Construction21 France – Pavillon de sanitaires du Chiro d’Itterbeek: https://www.construction21.org/france/case-studies/h/pavillon-de-sanitaires-du-chiro-d-itterbeek.html
- [S3] Rotor – Sanitary block for the Itterbeek Chiro: https://rotordb.org/en/projects/sanitary-block-itterbeek-chiro
- [S4] Construction21 Belgique – Pavillon de sanitaires du Chiro d’Itterbeek (Dilbeek): https://www.construction21.org/belgique/case-studies/h/pavillon-de-sanitaires-du-chiro-d-itterbeek-dilbeek.html
- [S5] Opalis – Rotor Deconstruction: https://opalis.eu/en/dealers/rotor-deconstruction
- [S6] Opalis – Franck Bricks: https://opalis.eu/en/dealers/franck-bricks
- [S7] Adokin – Bricks / Chiro d’Itterbeek summary: https://adokin.eu/fr/tag/briques/
