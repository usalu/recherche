---
id: "VDI_ZRE"
entity: "akteur"
node_kind: "core"
migration_status: "migrated_phase3_core_entities"
title: "VDI ZRE"
source_count: 1
legacy_paths:
  - "akteur\\03_forschung_lehre_wissenstransfer\\VDI_ZRE.md"
raw_targets:
  - "akteur/VDI_ZRE"
migration_actions:
  - "move_as_core"
risk_flags:
---
# VDI ZRE

## Migration

- Canonical target: akteur/VDI_ZRE
- Legacy source count: 1
- Semantic note: Akteurprofil. Kann spaeter mit Rollen, Beteiligungen, Orten und Plattformbetreiberschaft verknuepft werden.

## Legacy Content

### Legacy Source: akteur\03_forschung_lehre_wissenstransfer\VDI_ZRE.md

- Map action: move_as_core
- Target role in map: primary
- Raw mapped target: akteur/VDI_ZRE
- Original primary target: akteur/VDI_ZRE
- Original secondary targets: akteurtyp/forschung_lehre_wissenstransfer

## Verknüpfungen

**Übergeordnete Themen**
- Ressourceneffizienz in Unternehmen
- Kreislaufgerechtes Bauen
- Praxisbeispiele und Werkzeuge

**Verwandte Dateien**
- `akteur/Umweltbundesamt.md`
- `akteur/Restado.md`
- `methode/Ressourceneffizienz.md`
- `werkzeug/Bauteilboerse.md`
- `wirtschaft/Transaktionskosten.md`

**Akteurstyp**
- Praxis- und Transferakteur für Ressourceneffizienz

## Kurzüberblick zur Kategorie

**VDI Zentrum Ressourceneffizienz** ist im Kontext „Entwerfen mit Bestand“ / Wiederverwendung in der Architektur ein Praxis- und Transferakteur für Ressourceneffizienz. Das VDI Zentrum Ressourceneffizienz bereitet Ressourceneffizienzthemen für Unternehmen und Praxisakteure auf. Im Bauwesen ist es durch Themenseiten, Werkzeuge und gute Praxisbeispiele zu kreislaufgerechtem Bauen und Wiedernutzung von Baustoffen relevant.

Für das Repo ist dieser Akteur besonders wichtig, weil die Wiederverwendung von Bauteilen nicht nur durch einzelne Produkte entsteht, sondern durch ein Zusammenspiel aus politischem Rahmen, Planungskultur, Daten, Materiallogistik, Rückbau, Qualitätssicherung und Nachfrage. Die Datei ist als Akteursprofil zu lesen: Sie beschreibt Rolle, Nutzen, Grenzen und sinnvolle Querverbindungen, ersetzt aber keine projektspezifische Prüfung von Recht, Technik, Schadstoffen, Kosten oder Verfügbarkeit.

## Zentrale Unterthemen

- Kreislaufgerechtes Bauen
- Ressourceneffizienz in Unternehmen
- Gute Praxisbeispiele
- Restmengen, Rückbauprodukte und Plattformen
- Abgrenzung von Ressourceneffizienz, Recycling und ReUse

## Wichtige Dateien dieser Kategorie mit je 1–3 Sätzen Einordnung

- methode/Ressourceneffizienz.md — kann VDI-ZRE-Definitionen und Werkzeuge aufnehmen.
- wirtschaft/Transaktionskosten.md — erklärt, warum Restmengen trotz Materialwert schwer in Projekte gelangen.
- werkzeug/Bauteilboerse.md — verbindet Plattformbeispiele mit praktischen Hürden.

Weitere Anschlussdateien sollten den Akteur nicht nur erwähnen, sondern prüfen, welche konkrete Funktion er im ReUse-Prozess übernimmt: politischer Druck, technische Bewertung, Marktplatz, physisches Lager, Planungskompetenz, Forschung, Zertifizierung oder kommunale Infrastruktur.

## Relevanz für Wiederverwendung / Entwerfen mit Bestand

- Übersetzt ökologische ReUse-Ziele in betriebliche Argumente: weniger Primärmaterial, weniger Abfall, geringere Kosten und bessere Ressourcennutzung.
- Dokumentiert Beispiele wie restado und macht dadurch Plattform- und Restmengenlogiken sichtbar.
- Ist besonders nützlich für KMU, Handwerk, Bauunternehmen und praxisnahe Forschung.

Die Einordnung ist bewusst nicht auf „Bauteile kaufen“ reduziert. Gerade in Deutschland entstehen ReUse-Potenziale häufig vor dem eigentlichen Materialhandel: durch Erhaltungsentscheidungen, frühe Zielvereinbarungen, selektiven Rückbau, Gebäuderessourcenpässe, kommunale Beschaffung, lokale Materialorte und eine Planungskultur, die mit Verfügbarkeit, Patina und Unregelmäßigkeit umgehen kann.

## Querverbindungen zu anderen Kategorien

- Zu Restado: VDI ZRE nutzt restado als Praxisbeispiel für Wiedernutzung von Baustoffen.
- Zu UBA: UBA liefert den umweltpolitischen Rahmen, VDI ZRE die Unternehmensperspektive.
- Zu Bauteilbörsen: VDI-ZRE-Beispiele helfen, physische und digitale Modelle zu vergleichen.

Für die Arbeit im Repo sollte der Akteur mit Methoden-, Werkzeug-, Material-, Rechts- und Wirtschaftlichkeitsdateien verknüpft werden. Besonders wichtig ist die Unterscheidung zwischen:
- **Diskurs / Policy**: schafft Legitimation und politische Forderungen.
- **Daten / Bewertung**: macht Ressourcen sichtbar und vergleichbar.
- **Markt / Logistik**: bringt Angebot und Nachfrage zusammen.
- **Planung / Entwurf**: integriert verfügbare Bauteile in Räume, Tragwerke und Details.
- **Nachweis / Recht**: entscheidet, ob ein Bauteil genehmigungs- und haftungsfähig wiederverwendet werden kann.

## Offene Lücken / Ausbaufelder

- Praxisbeispiele können Erfolgsfälle überbetonen; Daten zu Scheitern und Kosten fehlen oft.
- Ressourceneffizienz ist breiter als direkte Wiederverwendung.
- Genehmigungsrelevante Bauprodukte benötigen zusätzliche technische Nachweise.

Zusätzlich sollte bei jeder späteren Vertiefung geprüft werden, ob die Angaben auf offiziellen Quellen, unabhängigen Evaluationen, Projektpublikationen oder Eigenkommunikation beruhen. Eigenangaben von Plattformen, Büros und Herstellern sind nützlich, müssen aber bei CO₂-Einsparungen, Wiederverwendungsquoten, Preisen und Skalierbarkeit kritisch markiert werden.

## Quellen / Bezugslogik

- https://www.ressource-deutschland.de/
- https://www.ressource-deutschland.de/themen/bauwesen/kreislaufgerechtes-bauen/
- https://www.ressource-deutschland.de/werkzeuge/ressourceneffizienz-in-der-praxis/gute-praxis-beispiele/suchergebnis/wiedernutzung-von-baustoffen/440/
- https://www.bmuv.de/themen/nachhaltigkeit-digitalisierung/ressourceneffizienz
