...
- **3,043 nodes, 9,349 edges, 0 dangling endpoints, 0 type mismatches.**
- **52** distinct `relation` tokens in `clean_confirmed_edges.csv`. Gap batches completed:
  ...
  - **50r** `has_bauobjektklasse` / `has_bauobjektrolle` / `has_bauobjektstatus` / `has_nutzung` / `has_bausystem` (bauobjekt, Konstruktion tokens) / `has_bauaufgabe_intervention`: 531 edges
  ...
  - **50y** `involves_foerderprogramm` / `has_programm_kontext`: 24 edges (Gebäude full-text scan → `foerderprogramm/*` and `programm_kontext/*` on `fallstudie`; Neo4j fold `GEHÖRT_ZU { rolle: programm }`)
