...
- Open question: should `registry_derived` keep `confidence='belegt'`? The actor master file is human-maintained and arguably belegt-grade for actor identity, but not for project participation. **Decision needed (D1).**
...
|---|---|---|---|
| D1 | Keep `evidence_confidence='belegt'` on registry-derived edges, or downgrade to `teilweise_belegt`? | Downgrade. Registry data is name-level belegt, not project-participation belegt. | R1 |
