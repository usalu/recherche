"""Apply Germany reuse bubble evidence-backed patches to mit-bestand."""
...
PHASES = [
    ...
    "phase2_bauteilboerse_hannover.patch.jsonl",
    "phase2b_hdm_civic_hub.patch.jsonl",
    "phase1c_evidence_hardening.patch.jsonl",
]

CONNECTIVITY = [
    (
        "T0_concular_verbunden",
        """
    ...
    (
        "T5_hdm_kunst_stoffe",
        """
        MATCH (h:Akteur {id:'haus_der_materialisierung'})-[r:VERBUNDEN_MIT_AKTEUR]-(k:Akteur {id:'kunst_stoffe_ev'})
        RETURN count(r) AS value
        """,
    ),
    (
        "T6_hdm_research_cluster",
        """
        MATCH (h:Akteur {id:'haus_der_materialisierung'})-[r:VERBUNDEN_MIT_AKTEUR]-(a:Akteur)
        WHERE a.id IN ['material_mafia','circular_berlin','tu_berlin','kunst_stoffe_ev']
        RETURN collect(DISTINCT a.id) AS value
        """,
    ),
]
