"""Apply Swiss reuse bubble evidence-backed patches to mit-bestand."""
...
PHASES = [
    ...
    "phase1_enrichment_connectivity.patch.jsonl",
    "phase2_new_nodes.patch.jsonl",
    "phase3_supply_chain.patch.jsonl",
]
...
CONNECTIVITY = [
    ...
    (
        "T0_2_cirkla_belegt_in",
        """
        MATCH (c:Akteur {id: 'cirkla'})-[:BELEGT_IN]->(q:Quelle)
        RETURN count(q) AS value
        """,
    ),
    (
        "T1_k118_beteiligt",
        """
        MATCH (p:Projekt {id: 'p_k118_kopfbau_halle_118_winterthur'})<-[r:BETEILIGT_AN]-(a:Akteur)
        RETURN collect(a.id) AS value
        """,
    ),
    (
        "T2_gruner_useagain_path",
        """
        MATCH p = shortestPath(
          (g:Akteur {id: 'gruner_reuse_platform'})-[:VERBUNDEN_MIT_AKTEUR*..6]-(u:Akteur {id: 'useagain_bauteilclick'})
        )
