...
Assemble the final classification prompt for one batch:
    ...
    KLASSIFIKATION_TAXONOMIE.md   (the whole prompt, {{AKTEURSDATEN}} at the end)
  + batches/klass_<CC>_b<N>.md    (that batch's actor blocks)
...
`validate_klassifikation.py` extracts the legal role names from the same
taxonomy file, so prompt and validator cannot drift apart.

Usage:
    python assemble_klassifikation_prompt.py klass_BE_b1.md > prompt.md
    python assemble_klassifikation_prompt.py --all        # writes prompts/*.md
"""
import os, sys, glob
...
PROMPTDIR = os.path.join(BASE, "prompts")

PLACEHOLDER = "{{AKTEURSDATEN}}"
...
def main():
    ...
    if args[0] == "--all":
        ...
        n = 0
        for p in sorted(glob.glob(os.path.join(BATCHDIR, "klass_*.md"))):
            name = os.path.basename(p)
            out = os.path.join(PROMPTDIR, name.replace("klass_", "prompt_"))
