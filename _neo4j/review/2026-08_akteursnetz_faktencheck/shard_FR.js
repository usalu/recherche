export const meta = {
  name: 'faktencheck-shard-FR',
  description: 'Live re-verification of drawn actor-network nodes/edges for FR',
  phases: [{ title: 'Shard-FR', detail: '16 agents, one evidence-clustered packet each' }],
}
...
log(`FR: ${packets.length} packets, ${packets.reduce((n,p)=>n+p.nodes.length,0)} nodes, ` +
...
Liefere das strukturierte Objekt. ${p.nodes.length} Knoten, ${p.edges.length} Kanten.`,
    { label: `FR:${p.packet_id}`, phase: 'Shard-FR', model: 'sonnet', effort: 'medium', schema: SHARD_SCHEMA })
...
log(`FR: ${nodes.length} Knotenurteile, ${edges.length} Kantenurteile, ${gaps.length} ungeprueft`)
log(`FR: kern=${nodes.filter(n=>n.actor_degree==='kern').length} ` +
    `bezug=${nodes.filter(n=>n.actor_degree==='bezug').length} ` +
    `ohne_beleg=${nodes.filter(n=>n.actor_degree==='ohne_beleg').length}`)

return { ccs: ["FR"], packets: ok, nodes, edges, ungeprueft: gaps, ausgefallen: lost.map(p => p.packet_id) }
