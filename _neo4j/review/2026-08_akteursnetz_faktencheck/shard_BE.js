export const meta = {
  name: 'faktencheck-shard-BE',
  description: 'Live re-verification of drawn actor-network nodes/edges for BE',
  phases: [{ title: 'Shard-BE', detail: '16 agents, one evidence-clustered packet each' }],
}
...
log(`BE: ${packets.length} packets, ${packets.reduce((n,p)=>n+p.nodes.length,0)} nodes, ` +
...
Liefere das strukturierte Objekt. ${p.nodes.length} Knoten, ${p.edges.length} Kanten.`,
    { label: `BE:${p.packet_id}`, phase: 'Shard-BE', model: 'sonnet', effort: 'medium', schema: SHARD_SCHEMA })
...
log(`BE: ${nodes.length} Knotenurteile, ${edges.length} Kantenurteile, ${gaps.length} ungeprueft`)
log(`BE: kern=${nodes.filter(n=>n.actor_degree==='kern').length} ` +
    `bezug=${nodes.filter(n=>n.actor_degree==='bezug').length} ` +
    `ohne_beleg=${nodes.filter(n=>n.actor_degree==='ohne_beleg').length}`)

return { ccs: ["BE"], packets: ok, nodes, edges, ungeprueft: gaps, ausgefallen: lost.map(p => p.packet_id) }
