export const meta = {
  name: 'faktencheck-shard-DK-SE',
  description: 'Live re-verification of drawn actor-network nodes/edges for DK-SE',
  phases: [{ title: 'Shard-DK-SE', detail: '15 agents, one evidence-clustered packet each' }],
}
...
log(`DK-SE: ${packets.length} packets, ${packets.reduce((n,p)=>n+p.nodes.length,0)} nodes, ` +
...
Liefere das strukturierte Objekt. ${p.nodes.length} Knoten, ${p.edges.length} Kanten.`,
    { label: `DK-SE:${p.packet_id}`, phase: 'Shard-DK-SE', model: 'sonnet', effort: 'medium', schema: SHARD_SCHEMA })
...
log(`DK-SE: ${nodes.length} Knotenurteile, ${edges.length} Kantenurteile, ${gaps.length} ungeprueft`)
log(`DK-SE: kern=${nodes.filter(n=>n.actor_degree==='kern').length} ` +
    `bezug=${nodes.filter(n=>n.actor_degree==='bezug').length} ` +
    `ohne_beleg=${nodes.filter(n=>n.actor_degree==='ohne_beleg').length}`)

return { ccs: ["DK", "SE"], packets: ok, nodes, edges, ungeprueft: gaps, ausgefallen: lost.map(p => p.packet_id) }
