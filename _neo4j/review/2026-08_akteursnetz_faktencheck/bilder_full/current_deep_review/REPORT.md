...
Open `index.html` or use `http://127.0.0.1:8766/` while the local review server is running. Filter by country or by `logo`/`none`.
...
Official provenance alone is not a reuse licence. The full publication gate is documented in `../CURRENT_IMAGE_RIGHTS_AUDIT.md`; blocked logos must receive written approval or be set to `none` before release.
