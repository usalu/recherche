export const meta = {
  name: 'faktencheck-shard-GB',
  description: 'Live re-verification of drawn actor-network nodes/edges for GB',
  phases: [{ title: 'Shard-GB', detail: '16 agents, one evidence-clustered packet each' }],
}
...
log(`GB: ${packets.length} packets, ${packets.reduce((n,p)=>n+p.nodes.length,0)} nodes, ` +
...
Liefere das strukturierte Objekt. ${p.nodes.length} Knoten, ${p.edges.length} Kanten.`,
    { label: `GB:${p.packet_id}`, phase: 'Shard-GB', model: 'sonnet', effort: 'medium', schema: SHARD_SCHEMA })
...
log(`GB: ${nodes.length} Knotenurteile, ${edges.length} Kantenurteile, ${gaps.length} ungeprueft`)
log(`GB: kern=${nodes.filter(n=>n.actor_degree==='kern').length} ` +
    `bezug=${nodes.filter(n=>n.actor_degree==='bezug').length} ` +
    `ohne_beleg=${nodes.filter(n=>n.actor_degree==='ohne_beleg').length}`)

return { ccs: ["GB"], packets: ok, nodes, edges, ungeprueft: gaps, ausgefallen: lost.map(p => p.packet_id) }
