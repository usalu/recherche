export const meta = {
  name: 'faktencheck-shard-DE',
  description: 'Live re-verification of drawn actor-network nodes/edges for DE',
  phases: [{ title: 'Shard-DE', detail: '17 agents, one evidence-clustered packet each' }],
}
...
log(`DE: ${packets.length} packets, ${packets.reduce((n,p)=>n+p.nodes.length,0)} nodes, ` +
...
Liefere das strukturierte Objekt. ${p.nodes.length} Knoten, ${p.edges.length} Kanten.`,
    { label: `DE:${p.packet_id}`, phase: 'Shard-DE', model: 'sonnet', effort: 'medium', schema: SHARD_SCHEMA })
...
log(`DE: ${nodes.length} Knotenurteile, ${edges.length} Kantenurteile, ${gaps.length} ungeprueft`)
log(`DE: kern=${nodes.filter(n=>n.actor_degree==='kern').length} ` +
    `bezug=${nodes.filter(n=>n.actor_degree==='bezug').length} ` +
    `ohne_beleg=${nodes.filter(n=>n.actor_degree==='ohne_beleg').length}`)

return { ccs: ["DE"], packets: ok, nodes, edges, ungeprueft: gaps, ausgefallen: lost.map(p => p.packet_id) }
