export const meta = {
  name: 'faktencheck-shard-CH',
  description: 'Live re-verification of drawn actor-network nodes/edges for CH',
  phases: [{ title: 'Shard-CH', detail: '13 agents, one evidence-clustered packet each' }],
}
...
log(`CH: ${packets.length} packets, ${packets.reduce((n,p)=>n+p.nodes.length,0)} nodes, ` +
...
Liefere das strukturierte Objekt. ${p.nodes.length} Knoten, ${p.edges.length} Kanten.`,
    { label: `CH:${p.packet_id}`, phase: 'Shard-CH', model: 'sonnet', effort: 'medium', schema: SHARD_SCHEMA })
...
log(`CH: ${nodes.length} Knotenurteile, ${edges.length} Kantenurteile, ${gaps.length} ungeprueft`)
log(`CH: kern=${nodes.filter(n=>n.actor_degree==='kern').length} ` +
    `bezug=${nodes.filter(n=>n.actor_degree==='bezug').length} ` +
    `ohne_beleg=${nodes.filter(n=>n.actor_degree==='ohne_beleg').length}`)

return { ccs: ["CH"], packets: ok, nodes, edges, ungeprueft: gaps, ausgefallen: lost.map(p => p.packet_id) }
