export const meta = {
  name: 'faktencheck-shard-FI-NO',
  description: 'Live re-verification of drawn actor-network nodes/edges for FI-NO',
  phases: [{ title: 'Shard-FI-NO', detail: '13 agents, one evidence-clustered packet each' }],
}
...
log(`FI-NO: ${packets.length} packets, ${packets.reduce((n,p)=>n+p.nodes.length,0)} nodes, ` +
...
Liefere das strukturierte Objekt. ${p.nodes.length} Knoten, ${p.edges.length} Kanten.`,
    { label: `FI-NO:${p.packet_id}`, phase: 'Shard-FI-NO', model: 'sonnet', effort: 'medium', schema: SHARD_SCHEMA })
...
log(`FI-NO: ${nodes.length} Knotenurteile, ${edges.length} Kantenurteile, ${gaps.length} ungeprueft`)
log(`FI-NO: kern=${nodes.filter(n=>n.actor_degree==='kern').length} ` +
    `bezug=${nodes.filter(n=>n.actor_degree==='bezug').length} ` +
    `ohne_beleg=${nodes.filter(n=>n.actor_degree==='ohne_beleg').length}`)

return { ccs: ["FI", "NO"], packets: ok, nodes, edges, ungeprueft: gaps, ausgefallen: lost.map(p => p.packet_id) }
