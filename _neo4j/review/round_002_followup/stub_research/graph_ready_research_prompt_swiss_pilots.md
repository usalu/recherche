# Graph-Ready Research Prompt — Swiss Circular Construction Pilots
...
1. `Schaerenmoosstrasse_Zuerich.md`
2. `UMAR_Unit_NEST_Empa_Duebendorf.md`
3. `ELEMENTA_Walkeweg_Basel.md`
...
Research the following three physical building case studies.
...
| Actor | Status |
|---|---|
| Daniel Hoffmann | already linked graph node |
...
| Actor | Status |
|---|---|
| Werner Sobek | already linked graph node |
| Dirk E. Hebel | already linked graph node |
...
Produce `ELEMENTA_Walkeweg_Basel.md` using the exact dossier template above.

If the project cannot be unambiguously identified, still produce the file and mark `identified_project` as `uncertain` or `no`. Document all candidate projects and negative findings.
